$(document).ready(function () { //document is a reference to an actual html element, the method ready
    var count = 0;
    var time = 0;
    var result = null;
    ...
    var jokes = ["You're so slow, the bird grew a beard while racing... go look..., GOTCHA!!! didn't I?",
...
$(window).keydown(function (e) {
    ...
    if (e.keyCode === 40) {
        if (position.top + $('.bird').height() < window.innerHeight) {
            $('.bird').css('top', Math.min(position.top + 50, window.innerWidth - $('.bird').height()));
        }
    }
})
